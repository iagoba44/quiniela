...
// Mock data generator for when external APIs are unconfigured or unavailable
...
export async function getBajasForTeam(team: string): Promise<BajasPayload> {
  ...
  // 2. Cascade Calls with silent fallbacks
  if (process.env.API_FOOTBALL_KEY) {
    try {
      result = await fetchApiFootball(team);
      sourceUsed = 'API-Football';
    } catch (e1: any) {
      console.debug(`[Orchestrator] API-Football unavailable for ${team}`);
    }
  }
